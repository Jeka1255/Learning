...
def foo7(df_age,df_gender,df_os):
    #TASK 7
...
#dag = DAG('dag_orlova_3', default_args=default_args, schedule_interval=schedule_interval)

with DAG('dag_orlova_3',default_args=default_args, schedule_interval=schedule_interval, catchup=False) as dag:
    ...
    t7 = PythonOperator(task_id='foo7',  # Название таска
                                ...
                                op_args = [foo4(foo3(foo1(),foo2())),foo5(foo3(foo1(),foo2())),foo6(foo3(foo1(),foo2()))],
                                dag=dag)  # Параметры DAG

    t8 = PythonOperator(task_id='into_base',  # Название таска
                                python_callable=into_base,  # Название функции
                                op_args = [foo7(foo4(foo3(foo1(),foo2())),foo5(foo3(foo1(),foo2())),foo6(foo3(foo1(),foo2())))],
                                dag=dag)  # Параметры DAG
